...
unsigned short crc_ccitt(unsigned short crc, void *buf, int len)
{
	...
	while (len--)
		crc = crc_ccitt_byte(crc, *p++);
	return crc;
}


/** Table of CRC constants - implements x^16+x^12+x^5+1 */
static const unsigned short crc16_xmodem_table[] = {
...
/**
* @brief crc16 calculation with software
*
* @param buf   data in buffer.
*
* @param len   data in length in bytes, must be 4bytes align.
*
* @return   crc value, 32 bit.
*
*/
unsigned short crc16_xmodem(unsigned short crc, void *buf, int len)
{
	...
	for (i = 0; i < len; i++)
	{
		cksum = crc16_xmodem_table[((cksum >> 8) ^ *p++) & 0xFF] ^ (cksum << 8);
	}
	return cksum;
}
