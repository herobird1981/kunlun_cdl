		...
		struct usb_function *function)
{
	...
	dev_dbg(NULL, "adding '%s'/%p to config '%s'/%p\n",
		...
		struct usb_configuration *config)
{
	...
	dev_dbg(NULL, "%s: adding config #%u '%s'/%p\n", __func__,
	...
	if (status < 0) {
		...
		dev_dbg(NULL, "cfg %d/%p speeds:%s%s\n",
		...
		for (i = 0; i < MAX_CONFIG_INTERFACES; i++) {
			...
			if (!f)
				continue;
			dev_dbg(NULL, "%s: interface %d = %s/%p\n",
			      __func__, i, f->name, f);
		}
	}
	...
	if (status)
		dev_dbg(NULL, "added config '%s'/%u --> %d\n", config->label,
		...
		unsigned n_strings)
{
	...
	uc = memalign(CONFIG_SYS_CACHELINE_SIZE, mem);
	if (!uc)
...
composite_setup(struct usb_gadget *gadget, const struct usb_ctrlrequest *ctrl)
{
	...
	default:
unknown:
		dev_dbg(NULL, "non-core control req%02x.%02x v%04x i%04x l%d\n",
